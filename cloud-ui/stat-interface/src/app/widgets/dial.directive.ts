...
export class DialDirective implements OnInit, OnChanges {
  element: HTMLElement;
  @Input() value: number;
  @Input() options: any;
  ...
  draw() {
    ...
    if (that.options.animate.enabled) {
      that.valueElem.transition().ease(that.options.animate.ease).duration(that.options.animate.duration).tween('', function(d) {
        var i = d3.interpolate(
          that.valueToRadians(that.options.startAngle, 360),
          that.valueToRadians(that.value, that.options.max, that.options.endAngle, that.options.startAngle, that.options.min)
        );
        return (t) => {
          var val = i(t);
          d3.select(this).attr('d', that.valueArc.endAngle(val));
          d3.select(this).attr('d', that.changeArc.endAngle(val));
        };
      });
    } else {
    ...
    function interaction(x, y, isFinal) {
      ...
      if (that.value >= that.options.min && that.value <= that.options.max) {
        ...
        d3.select(this).attr('d', that.valueArc);
        if (isFinal) {
          ...
          d3.select(this).attr('d', that.changeArc);
        }
